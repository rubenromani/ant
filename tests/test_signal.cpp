...
TEST_F(SignalTest, ConnectLambda) {
  ...
  auto conn = sig.connect([&](int value) { received_value = value; });
...
TEST_F(SignalTest, ConnectMultipleLambdas) {
  ...
  auto conn1 =
      sig.connect([&](int value) { received_values.push_back(value * 2); });

  auto conn2 =
      sig.connect([&](int value) { received_values.push_back(value * 3); });

  EXPECT_EQ(sig.slot_count(), 2);

  sig.emit(10);
  EXPECT_EQ(received_values.size(), 2);
  EXPECT_TRUE(std::find(received_values.begin(), received_values.end(), 20) !=
              received_values.end());
  EXPECT_TRUE(std::find(received_values.begin(), received_values.end(), 30) !=
              received_values.end());
}
...
TEST_F(SignalTest, ExceptionSafety) {
  ...
  auto conn1 =
      sig.connect([](int) { throw std::runtime_error("test exception"); });

  auto conn2 = sig.connect([&](int) { safe_call_count++; });
